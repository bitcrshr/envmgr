...
func (vq *VariableQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (vq *VariableQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (vq *VariableQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (vq *VariableQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (vq *VariableQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (vq *VariableQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := vq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (vq *VariableQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Variable, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, variable.ForeignKeys...)
	}
...
func (vq *VariableQuery) loadEnvironment(ctx context.Context, query *EnvironmentQuery, nodes []*Variable, init func(*Variable), assign func(*Variable, *Environment)) error {
	...
	for i := range nodes {
		if nodes[i].environment_vars == nil {
			continue
		}
		fk := *nodes[i].environment_vars
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "environment_vars" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
