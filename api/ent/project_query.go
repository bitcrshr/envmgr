...
//		GroupBy(project.FieldDisplayName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Project.Query().
//		Select(project.FieldDisplayName).
//		Scan(ctx, &v)
...
func (pq *ProjectQuery) loadEnvironments(ctx context.Context, query *EnvironmentQuery, nodes []*Project, init func(*Project), assign func(*Project, *Environment)) error {
	...
	for _, n := range neighbors {
		fk := n.project_environments
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_environments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_environments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
