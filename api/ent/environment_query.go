...
func (eq *EnvironmentQuery) QueryVars() *VariableQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, environment.VarsTable, environment.VarsColumn),
		)
		fromU = sqlgraph.SetNeighbors(eq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (eq *EnvironmentQuery) WithVars(opts ...func(*VariableQuery)) *EnvironmentQuery {
	query := (&VariableClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EnvironmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Environment, error) {
	...
	if query := eq.withVars; query != nil {
		if err := eq.loadVars(ctx, query, nodes,
			func(n *Environment) { n.Edges.Vars = []*Variable{} },
			func(n *Environment, e *Variable) { n.Edges.Vars = append(n.Edges.Vars, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EnvironmentQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *Project)) error {
	...
	for i := range nodes {
		if nodes[i].project_environments == nil {
			continue
		}
		fk := *nodes[i].project_environments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_environments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (eq *EnvironmentQuery) loadVars(ctx context.Context, query *VariableQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *Variable)) error {
	...
	query.Where(predicate.Variable(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(environment.VarsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.environment_vars
		if fk == nil {
			return fmt.Errorf(`foreign-key "environment_vars" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "environment_vars" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
